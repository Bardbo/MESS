...
class STGCN(nn.Module):
    ...
    def __init__(self, num_nodes, num_features, num_timesteps_input,
        ...
        self.fully1 = nn.Linear((num_timesteps_input - 2 * 5) * 64,
                               num_timesteps_output)
        self.agg = nn.Linear(num_nodes*2, num_nodes)
    ...
    def forward(self, A_hat1, A_hat2, X):
        ...
        out9 = torch.cat([out4, out8], dim=1)
        out10 = self.agg(out9.permute(0,2,1)).permute(0,2,1)
        return out10
...
#         plt.show()
    ...
    with open(f"save_pk/random_stgcn+_{epochs}_losses.pk", "wb") as fd:
        pk.dump((training_losses, validation_losses, validation_maes), fd)
    torch.save(net.state_dict(), f'save_pk/random_stgcn+_{epochs}.pkl')
    end_time = time.time()
