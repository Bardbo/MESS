...
# @Last Modified time: 2021-01-11 12:55:32
import math
import torch
...
from utils import load_data, load_A, get_normalized_adj, generate_dataset
...
class STGCN(nn.Module):
    ...
    def __init__(self, num_nodes, num_features, num_timesteps_input,
        ...
        self.fully = nn.Linear((num_timesteps_input - 2 * 5) * 64,
                               num_timesteps_output)
        self.A_hat = nn.Parameter(torch.randn(num_nodes, num_nodes))
...
#         plt.show()
    ...
    with open(f"save_pk/self_adj_stgcn_{epochs}_losses.pk", "wb") as fd:
        pk.dump((training_losses, validation_losses, validation_maes), fd)
    torch.save(net.state_dict(), f'save_pk/self_adj_stgcn_{epochs}.pkl')
    end_time = time.time()
